...
function IncidentRow({ incident }: { incident: Incident }) {
  ...
  return (
    <div className="flex items-center gap-3 p-3 rounded-lg border border-transparent hover:border-slate-700 hover:bg-slate-800/40 transition-all cursor-pointer">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className="text-white text-xs font-medium truncate">{incident.type}</span>
          <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${styles.dot}`} />
        </div>
        <div className="text-slate-500 text-xs truncate">{incident.zone} — {incident.description}</div>
      </div>
      <div className="flex items-center gap-2 flex-shrink-0">
        <span className="text-slate-500 text-xs font-mono">{incident.time}</span>
  ...
  return (
    <div className="rounded-xl border border-slate-800 p-5 h-full" style={{ background: '#0f1629' }}>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-white font-semibold text-sm">Incidents récents</h2>
        <button className="text-blue-400 text-xs hover:text-blue-300 transition-colors">Voir tout</button>
      </div>
