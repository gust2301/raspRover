...
export default function VehicleInspections() {
  ...
  useEffect(() => {
    if (!connected || (phase !== 'mapping' && phase !== 'learning')) return
    let disposed = false
  ...
  return <div className="space-y-5 max-w-7xl mx-auto">
    ...
    {phase === 'learning' && <div className="space-y-5">
      <section className="rounded-xl border border-blue-200 bg-blue-50 dark:border-blue-500/30 dark:bg-blue-500/5 p-4"><h2 className="text-slate-900 dark:text-white font-semibold">Étape 2 — Apprenez le tour du véhicule de référence</h2><p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Pilotez autour du véhicule, ou marchez lentement devant le rover avec « Suivez-moi ». À chaque vue importante, orientez la caméra, choisissez la zone puis enregistrez le point. Les coordonnées sont capturées automatiquement.</p></section>
      <div className="grid xl:grid-cols-[1fr_360px] gap-5">
        <div className="space-y-5"><LiveCamera streamUrl={streamUrl} /><MapPanel map={mapData} pose={pose} points={points} trail={followStatus.follow_me_trail} /></div>
        <aside className="space-y-5">
          <section className={`rounded-xl border p-4 space-y-3 ${followStatus.follow_me_active ? 'border-blue-300 bg-blue-50 dark:border-blue-500/40 dark:bg-blue-500/10' : 'border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900/50'}`}>
            <div className="flex items-center gap-2">
              <UserRound size={18} className="text-blue-600 dark:text-blue-400" />
